...
useEffect(() => {
  const unsubscribe = onSnapshot(collection(db, "sticky"), (snapshot) => {
  ...
  const handleSave = async (id: number, note: StickyNote) => {
    ...
    try {
      ...
      if (note.firestoreId) {
        await updateDoc(doc(db, "sticky", note.firestoreId), {
        ...
        const docRef = await addDoc(collection(db, "sticky"), {
          id: note.id.toString(),
          topic: note.topic.trim(),
          description: note.description.trim(),
          color: note.color,
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      if (noteToDelete?.firestoreId) {
        await deleteDoc(doc(db, "sticky", noteToDelete.firestoreId));
      }
                    ...
                    {note.isEditing ? (
                      ...
                      <>
                        <div className="flex flex-row items-center justify-between">
                          ...
                          <MdDeleteForever
                            className="group-hover:block hidden text-2xl hover:text-red-500 cursor-pointer"
                            onClick={(e) => {
                              e.stopPropagation();
                              handleDelete(note.id);
                            }}
